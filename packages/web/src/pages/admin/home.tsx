...
import { Tag, ArrowLeftRight, GitBranch, AlertTriangle, Users, Layers, Inbox, ArrowRight, CheckSquare, Network, BookOpen } from 'lucide-react';
...
const TABS: TabCard[] = [
  {
    ...
    description: 'Votre point d\'entrée quotidien. Agrège tout ce qui demande votre attention : fragments à valider, métadonnées en attente, contradictions à résoudre.',
    cta: 'Ouvrir l\'Inbox',
    link: '/admin/inbox',
    available: false,
  },
  {
    ...
    description: 'Gère le vocabulaire du système : sujets, entités, tags. "À valider" traite les nouvelles propositions du LLM. "Référentiel" permet la maintenance du catalogue actif.',
  ...
  },
  {
    ...
    description: 'Quand un nouveau fragment ressemble fortement à un existant, le système suggère un remplacement. Vous comparez côte à côte et décidez : remplacer, coexister ou rejeter.',
  ...
  },
  {
    ...
    description: 'Le système détecte des relations sémantiques entre fragments (détaille, illustre, complète). Vous validez pour enrichir le knowledge graph.',
  ...
  },
  {
    ...
    description: 'Détecte automatiquement les fragments qui se contredisent (deux SLA différents, deux prix différents). Critique pour garantir la cohérence des documents composés.',
  ...
  },
  {
    ...
    description: 'Liste des utilisateurs, gestion des rôles, désactivation de comptes, audit de qui a fait quoi.',
  ...
  },
  {
    ...
    description: 'Les fragments peuvent être regroupés en collections par équipe, projet ou client. Chaque collection peut être personnelle, partagée ou en lecture seule.',
  ...
  },
  {
    ...
    description: 'Graphe interactif des fragments validés. Les nœuds sont les fragments approuvés. Les liens implicites relient les fragments partageant des entités ou proches sémantiquement (Milvus). Les liens explicites sont les relations validées dans le tab Relations.',
...
const WORKFLOW_STEPS = [
  { step: '1', label: 'Ingestion', detail: 'Un utilisateur uploade un document source avec hints optionnels', tab: 'Harvest' },
  { step: '2', label: 'Validation candidats', detail: 'Vous validez les fragments extraits du document', tab: 'Harvest' },
  { step: '3', label: 'Validation métadonnées', detail: 'Vous validez les nouveaux tags / entités proposés par le LLM', tab: 'Metadata' },
  { step: '4', label: 'Enrichissement', detail: 'Validez les relations, remplacements et contradictions détectés', tab: 'Tabs dédiés' },
  { step: '5', label: 'Production', detail: 'Les fragments validés sont utilisables pour composer des documents', tab: 'Composition' },
];

const QUICK_START = [
  { label: 'Vérifiez le référentiel', detail: 'Metadata > Référentiel — vérifiez que sujets, entités et tags correspondent à votre contexte métier.' },
  { label: 'Testez une ingestion', detail: 'Uploadez un document que vous connaissez. Pré-remplissez les hints (domaine, fonction, entités) pour réduire la validation.' },
  { label: 'Lisez le bilan d\'ingestion', detail: 'Après l\'ingestion, le système affiche un bilan. Plus vous avez fourni de hints, plus de métadonnées sont auto-validées.' },
  { label: 'Validez les fragments candidats', detail: 'Tab Harvest — pour les fragments "tout vert", validation rapide. Pour ceux avec warnings, lecture attentive.' },
  { label: 'Validez les métadonnées émergentes', detail: 'Metadata > À valider — approuvez, fusionnez avec des existants ou rejetez les nouvelles propositions du LLM.' },
  { label: 'Traitez les détections automatiques', detail: 'Remplacements, Relations, Contradictions — traitez les détections liées à votre ingestion.' },
  { label: 'Vérifiez en composition', detail: 'Créez un document de test dans la vue Composition et vérifiez que les fragments validés apparaissent dans les recommandations.' },
];
...
const METADATA_GLOSSARY = [
  {
    ...
    values: 'twake-mail · twake-calendar · twake-drive · twake-chat · linshare · lincloud · linto · openrag · linagora-corp · other',
  ...
  },
  {
    ...
    values: 'argument · description · pricing · clause · faq · introduction · conclusion · temoignage · reference-technique · cas-usage · methodology · engagement · bio',
  ...
  },
  {
    ...
    values: 'clients · products · technologies · partners · certifications · regulations — référentiel validé',
  ...
  },
  {
    ...
    values: 'draft (extrait, non vérifié) · reviewed (vérifié par un humain) · approved (validé, utilisable en composition)',
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-10">
      ...
      <section>
        ...
        <p className="text-muted-foreground leading-relaxed max-w-2xl">
          Fragmint assemble des documents par recomposition de fragments versionnés.
          Cette console permet de maintenir la qualité du corpus : valider les métadonnées proposées
          par l'IA, gérer les remplacements de contenu obsolète, résoudre les contradictions et
          superviser l'évolution du référentiel.
        </p>
      </section>
          ...
          <div className="flex items-start justify-between">
            {WORKFLOW_STEPS.map((s) => (
              <div key={s.step} className="flex flex-col items-center flex-1 px-1 text-center">
                ...
                <span className="text-xs text-blue-600 dark:text-blue-400 mt-1 font-medium">{s.tab}</span>
              </div>
            ))}
          </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {available.map((tab) => <TabCardItem key={tab.title} tab={tab} />)}
        </div>
        {coming.length > 0 && (
          <div className="mt-6">
            <p className="text-xs font-medium text-muted-foreground mb-3 uppercase tracking-wide">Bientôt disponible</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 opacity-50">
              {coming.map((tab) => <TabCardItem key={tab.title} tab={tab} disabled />)}
            </div>
          </div>
        )}
      </section>
      ...
      <section>
        ...
        <p className="text-xs text-muted-foreground mb-4">
          Chaque fragment porte ces 8 axes de classification. Comprendre leur rôle permet de valider et corriger plus vite.
        </p>
        <div className="space-y-2">
            ...
            <details key={item.code} className="group border rounded-lg">
              <summary className="flex items-center justify-between px-4 py-3 cursor-pointer select-none list-none hover:bg-muted/40 rounded-lg transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <code className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded">{item.code}</code>
                </div>
              ...
              <div className="px-4 pb-3 pt-1 border-t space-y-2">
                ...
                <div>
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide mb-1">Valeurs</p>
                  <p className="text-xs text-muted-foreground">{item.values}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground font-medium uppercase tracking-wide mb-1">Exemple</p>
...
function TabCardItem({ tab, disabled }: { tab: TabCard; disabled?: boolean }) {
  const Icon = tab.icon;
  return (
    <div className={cn('border rounded-lg p-4 flex flex-col gap-3', !disabled && 'hover:shadow-sm transition-shadow bg-card')}>
