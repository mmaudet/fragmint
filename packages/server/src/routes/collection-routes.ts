...
export function collectionRoutes(
  ...
  app.post('/v1/collections', { preHandler: [authenticate] }, async (request, reply) => {
    ...
    if (body.type === 'system' && request.user.role !== 'admin') {
      return reply
        .status(403)
        .send({ data: null, meta: null, error: 'Only admins can create system collections' });
    }
  ...
  app.get(
    '/v1/collections/:slug',
    { preHandler: [authenticate, requireCollectionRole('reader')] },
    async (request) => {
  ...
  app.post(
    ...
    { preHandler: [authenticate, requireCollectionRole('manager')] },
    async (request, reply) => {
  ...
  app.delete(
    '/v1/collections/:slug/members/:userId',
    { preHandler: [authenticate, requireCollectionRole('manager')] },
    async (request) => {
