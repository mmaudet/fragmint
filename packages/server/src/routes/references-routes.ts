...
export function referencesRoutes(
  ...
  app.get(
    ...
    async (_req, reply) => {
      ...
      return reply.send({
        data: rows.map((r) => ({ slug: r.slug, label: r.label, description: r.description, usage_count: r.usageCount })),
  ...
  app.get(
    ...
    async (_req, reply) => {
      const rows = await db
        ...
        .orderBy(desc(fragmentTags.usageCount));

      return reply.send({
        data: rows.map((r) => ({ slug: r.slug, label: r.label, category: r.category, usage_count: r.usageCount })),
        meta: { count: rows.length },
        error: null,
      });
    },
