...
export function adminMetadataLookupRoutes(
  ...
  app.get(
    ...
    async (request, reply) => {
      const { kind, q = '', limit: limitStr } = (request.query ?? {}) as {
        kind?: string;
        q?: string;
        limit?: string;
      };
      ...
      if (kind === 'domain') {
        ...
        if (search) conditions.push(or(like(fragmentDomains.slug, `%${search}%`), like(fragmentDomains.label, `%${search}%`)));
        const rows = await db
          .select({ slug: fragmentDomains.slug, label: fragmentDomains.label })
          .from(fragmentDomains)
          .where(and(...conditions))
          .limit(maxResults);
        return reply.send({ data: rows.map((r) => ({ slug: r.slug, label: r.label })), meta: null, error: null });
      }

      if (kind === 'tag') {
        const conditions: any[] = [eq(fragmentTags.validated, 1)];
        if (search) conditions.push(or(like(fragmentTags.slug, `%${search}%`), like(fragmentTags.label, `%${search}%`)));
        const rows = await db
      ...
      if (kind === 'function') {
        ...
        if (search) conditions.push(or(like(fragmentFunctions.slug, `%${search}%`), like(fragmentFunctions.label, `%${search}%`)));
        const rows = await db
